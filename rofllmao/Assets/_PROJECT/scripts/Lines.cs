...
// latest version at: https://pastebin.com/8m69iTut -- added: Public Domain (2017/09/21)
public class Lines : MonoBehaviour {
	...
	public static LineRenderer MakeArrow(ref GameObject lineObject, Vector3 start, Vector3 end,
		...
		return MakeArrow(ref lineObject, new Vector3[] { start, end }, 2, arrowHeadSize, color, startSize, endSize);
	}
	public static LineRenderer MakeArrowBothEnds(ref GameObject lineObject, Vector3 start, Vector3 end,
		float arrowHeadSize = 4, Color color = default(Color), float startSize = 0.125f, float endSize = 0.125f) {
		return MakeArrowBothEnds(ref lineObject, new Vector3[] { start, end }, 2, arrowHeadSize, color, startSize, endSize);
	}
	public static LineRenderer MakeArrow(ref GameObject lineObject, Vector3[] points, int pointCount,
		float arrowHeadSize = 4, Color color = default(Color), float startSize = 0.125f, float endSize = 0.125f, Keyframe[] lineKeyFrames = null) {
	...
	public static LineRenderer MakeArrowBothEnds(ref GameObject lineObject, Vector3[] points, int pointCount,
		float arrowHeadSize = 4, Color color = default(Color), float startSize = 0.125f, float endSize = 0.125f) {
		LineRenderer lr = MakeArrow(ref lineObject, points, pointCount, arrowHeadSize, color, startSize, endSize, null);
		ReverseLineInternal(ref lr);
		Vector3[] p = new Vector3[lr.positionCount];
		lr.GetPositions(p);
		lr = MakeArrow(ref lineObject, p, p.Length, arrowHeadSize, color, endSize, startSize, lr.widthCurve.keys);
		ReverseLineInternal(ref lr);
		return lr;
	}
	...
	public static LineRenderer MakeArcArrow(ref GameObject lineObj,
		float angle, int pointCount, float arrowHeadSize = 4, Vector3 normal = default(Vector3), Vector3 firstPoint = default(Vector3),
		Vector3 center = default(Vector3), Color color = default(Color), float startSize = 0.125f, float endSize = 0.125f) {
		if(normal == default(Vector3)) { normal = Vector3.up; }
		if(center == default(Vector3) && firstPoint == default(Vector3)) { firstPoint = Vector3.right; }
		Vector3[] points = null;
		WriteArc(ref points, pointCount, normal, firstPoint, angle, center);
		return MakeArrow(ref lineObj, points, pointCount, arrowHeadSize, color, startSize, endSize);
	}

	public static LineRenderer MakeArcArrowBothEnds(ref GameObject lineObj,
		float angle, int pointCount, float arrowHeadSize = 4, Vector3 normal = default(Vector3), Vector3 firstPoint = default(Vector3),
		Vector3 center = default(Vector3), Color color = default(Color), float startSize = 0.125f, float endSize = 0.125f) {
		LineRenderer lr = MakeArcArrow(ref lineObj, angle, pointCount, arrowHeadSize, normal, firstPoint, center, color, startSize, endSize);
		ReverseLineInternal(ref lr);
		Vector3[] p = new Vector3[lr.positionCount];
		lr.GetPositions(p);
		lr = MakeArrow(ref lineObj, p, p.Length, arrowHeadSize, color, endSize, startSize, lr.widthCurve.keys);
		ReverseLineInternal(ref lr);
		return lr;
	}

	public static void DrawQuaternion(ref GameObject axisObj, ref GameObject angleObj, Quaternion quaternion, 
		Vector3 position=default(Vector3), Vector3 startPoint=default(Vector3), Quaternion orientation=default(Quaternion), 
		float arrowHeadSize = 2, Color color=default(Color), int arcPoints = 24, float lineSize = 0.125f) {
		if (orientation == default(Quaternion)) { orientation = Quaternion.identity; }
		if (startPoint == default(Vector3)) { startPoint = Vector3.forward; }
		float angle;
		Vector3 axis;
		quaternion.ToAngleAxis (out angle, out axis);
		Vector3 axisRotated = orientation * axis;
		Lines.MakeArrow (ref axisObj, position - axisRotated/2, position + axisRotated/2, arrowHeadSize, color, lineSize, lineSize);
		Lines.MakeArcArrow (ref angleObj, angle, arcPoints, arrowHeadSize, axisRotated, startPoint, position, color, lineSize, lineSize);
	}
}